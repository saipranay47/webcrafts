...
function CardUrl({ title, description, tags, username, image, profilepic, ...props }) {
  ...
  return (
    <div
      className="bg-[#202020] rounded-xl max-w-md w-full"
      ...
      <article className="group max-w-md relative bg-[#202020] rounded-xl w-full">
        {imageUrl && (
          <img
            alt="Lava"
            src={imageUrl}
            className="h-64 w-full rounded-xl object-cover shadow-xl transition group-hover:grayscale-[50%] shadow-[#30303030]"
          />
        )}
        <Transition
          show={isHovered}
